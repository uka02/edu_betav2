...
<head>
    ...
    <title>{{ __('lessons.edit_lesson') }}</title>
    ...
    <style>
        ...
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        .form-textarea:focus {
            ...
            background: linear-gradient(135deg, rgba(42, 54, 84, 0.8), rgba(31, 43, 74, 0.8));
            box-shadow: 0 0 0 3px rgba(168, 85, 247, 0.15), inset 0 2px 4px rgba(0, 0, 0, 0.2);
        }

        .form-input.input-error {
            border-color: rgba(239, 68, 68, 0.6) !important;
            box-shadow: 0 0 0 3px rgba(239, 68, 68, 0.15), inset 0 2px 4px rgba(0, 0, 0, 0.2) !important;
        }
        ...
        .builder-header {
            ...
            border-image: linear-gradient(90deg, rgba(168, 85, 247, 0.5), rgba(99, 102, 241, 0.3)) 1;
        }
        ...
        .segment-type-btn-icon { font-size: 20px; }
        .segment-type-btn-text { flex: 1; }
        .segment-type-btn-desc { font-size: 12px; color: var(--muted); margin-top: 2px; }
        ...
        .quiz-add-answer:hover { background: var(--accent-hover); }
        ...
        .quiz-answer-remove:hover { background: rgba(239, 68, 68, 0.1); }
        .quiz-hint { font-size: 12px; color: var(--muted); margin-top: 8px; }
...
<div class="app-shell">
        ...
        <header class="page-topbar">
            <div class="ptb-left">
                ...
                <div class="ptb-copy">
                    ...
                    <span class="ptb-title">{{ __('lessons.edit_lesson') }}</span>
                </div>
            </div>
            <div class="ptb-right">
    ...
    <div class="container">
        <div class="header">
            <h1 class="page-title">{{ __('lessons.edit_lesson') }}</h1>
        ...
        <form action="{{ route('lessons.update', $lesson) }}" method="POST" enctype="multipart/form-data" id="lessonForm">
            @csrf
            @method('PUT')
                ...
                <div class="content-area">
                    <div class="segment-container active" data-segment="0">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" id="title" name="title" class="form-input" value="{{ old('title', $lesson->title) }}" placeholder="{{ __('lessons.title_placeholder') }}" required>
                        ...
                        <div class="form-group">
                            ...
                            <select id="type" name="type" class="form-select" required>
                                ...
                                <option value="video" {{ old('type', $lesson->type) == 'video' ? 'selected' : '' }}>{{ __('lessons.video') }}</option>
                                <option value="text" {{ old('type', $lesson->type) == 'text' ? 'selected' : '' }}>{{ __('lessons.text') }}</option>
                                <option value="document" {{ old('type', $lesson->type) == 'document' ? 'selected' : '' }}>{{ __('lessons.document_type') }}</option>
                            </select>
                            @error('type')
                                <p class="form-error">{{ $message }}</p>
                            @enderror
                        ...
                        <div class="form-group" id="video-url-group" style="display: none;">
                            ...
                            <input type="url" id="video_url" name="video_url" class="form-input" value="{{ old('video_url', $lesson->video_url) }}" placeholder="{{ __('lessons.video_url_placeholder') }}">
                        ...
                        <div class="form-group" id="document-group" style="display: none;">
                            ...
                            <div class="file-input-wrapper">
                                <input type="file" id="document" name="document" class="file-input" accept=".pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx" data-existing-document="{{ $lesson->document_path ? '1' : '0' }}" data-existing-document-url="{{ $lesson->document_path ? Storage::url($lesson->document_path) : '' }}">
                                ...
                                <p class="file-name" id="document-name">{{ $lesson->document_path ? basename($lesson->document_path) : __('lessons.no_file_chosen') }}</p>
                            </div>
                        ...
                        <div class="form-group">
                            ...
                            <div class="file-input-wrapper">
                                <input type="file" id="thumbnail" name="thumbnail" class="file-input" accept="image/*" data-existing-thumbnail-url="{{ $lesson->thumbnail ? Storage::url($lesson->thumbnail) : '' }}">
                                ...
                                @if($lesson->thumbnail)
                                    <div class="existing-file-preview">{{ basename($lesson->thumbnail) }}</div>
                                @endif
                            </div>
                        ...
                        <div class="form-group">
                            ...
                            <div class="duration-inputs">
                                <div class="input-group">
                                    ...
                                    <input type="number" name="duration_hours" class="form-input" value="{{ old('duration_hours', intval($lesson->duration_minutes / 60)) }}" min="0" max="99">
                                </div>
                                <div class="input-group">
                                    <span class="input-label">{{ __('lessons.minutes') }}:</span>
                                    <input type="number" name="duration_minutes" class="form-input" value="{{ old('duration_minutes', $lesson->duration_minutes % 60) }}" min="0" max="59">
                                </div>
                            </div>
                        ...
                        <div class="form-group">
                            ...
                            <select id="difficulty" name="difficulty" class="form-select">
                                <option value="">--</option>
                                <option value="beginner" {{ old('difficulty', $lesson->difficulty) == 'beginner' ? 'selected' : '' }}>{{ __('lessons.beginner') }}</option>
                                <option value="intermediate" {{ old('difficulty', $lesson->difficulty) == 'intermediate' ? 'selected' : '' }}>{{ __('lessons.intermediate') }}</option>
                                <option value="advanced" {{ old('difficulty', $lesson->difficulty) == 'advanced' ? 'selected' : '' }}>{{ __('lessons.advanced') }}</option>
                            </select>
                            @error('difficulty')
                                <p class="form-error">{{ $message }}</p>
                            @enderror
                        ...
                        <div class="form-group">
                            ...
                            <select id="subject" name="subject" class="form-select">
                                @php($selectedSubject = \App\Models\Lesson::normalizeSubject(old('subject', $lesson->subject ?? \App\Models\Lesson::defaultSubject())))
                        ...
                        <div class="form-group">
                            <div class="checkbox-group">
                                <input type="checkbox" id="is_published" name="is_published" class="checkbox-input" value="1" {{ old('is_published', $lesson->is_published) ? 'checked' : '' }}>
                        ...
                        <div class="form-group">
                            <div class="checkbox-group">
                                <input type="checkbox" id="is_free" name="is_free" class="checkbox-input" value="1" {{ old('is_free', $lesson->is_free) ? 'checked' : '' }}>
    ...
    <script>
        ...
        @include('partials.lessons.builder-edit-renderers')
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            bindLessonBuilderFormHelpers({
                ...
                excludeId: document.querySelector('meta[name="lesson-id"]')?.content,
            ...
            bindLessonDraftSupport({
                storageKey: `lesson-builder-draft:edit:${document.querySelector('meta[name="lesson-id"]')?.content || 'new'}`,
                clearStorageKeysOnLoad: lessonDraftCleanupKeys,
                autosaveUrl: @js(route('lessons.autosave')),
                canAutosaveToServer: @json(! $lesson->is_published),
                initialLessonId: @json($lesson->id),
                lessonStorageKeyPrefix: 'lesson-builder-draft:edit:',
            });
        });

        @include('partials.lessons.builder-edit-segment-factories')

        @include('partials.lessons.builder-exam-question-helpers')

        @include('partials.lessons.builder-form-helpers')

        @include('partials.lessons.builder-edit-block-factories')
